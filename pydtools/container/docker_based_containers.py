...
def kube(args_string):
    image_name = utils.get_dtools_image_name("kube")
    dood_command = f"docker run -ti {utils.attach_work()} {utils.attach_git()} --rm -v /var/run/docker.sock:/var/run/docker.sock {args_string} {image_name} zsh"
    dind_command = f"docker run -d {utils.attach_work()} {utils.attach_git()} --privileged {args_string} {image_name}"
    run_docker_container(image_name, dood_command, dind_command)


def awskube(args_string):
    image_name = utils.get_dtools_image_name("awskube")
    dood_command = f"docker run -it {utils.attach_work()} {utils.attach_git()} -v {os.path.expanduser('~')}/.aws:/root/.aws -v /var/run/docker.sock:/var/run/docker.sock --rm {args_string} {image_name} zsh"
    dind_command = f"docker run -d {utils.attach_work()} {utils.attach_git()} -v {os.path.expanduser('~')}/.aws:/root/.aws --privileged {args_string} {image_name}"
    run_docker_container(image_name, dood_command, dind_command)
